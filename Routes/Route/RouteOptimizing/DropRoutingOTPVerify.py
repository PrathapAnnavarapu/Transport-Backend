...
def check_and_finalize_trip(vehicle_id, schedule_id):
    if not schedule_id:
        raise ValueError("❌ schedule_id is required to finalize trip billing.")

    trip_entries = DropRouting.query.filter_by(vehicle_id=vehicle_id).all()
    if not trip_entries:
        return False
    ...
    if all_dropped:
        print(f"✅ Trip completed for vehicle {vehicle_id}, schedule {schedule_id}")
...
def drop_offboard_employee():
    ...
    if routing.off_board_OTP == otp:
        ...
        # ✅ Trigger trip finalization only if schedule_id is valid
